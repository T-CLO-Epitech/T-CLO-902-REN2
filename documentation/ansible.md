...
- Installation de **Helm** et déploiement des services (**Traefik**)
...
| `kube2` | Worker | `role=kube2`      | `role=kube2:NoSchedule`           | Nœud worker applicatif |
...
| `Install_Helm` | Master uniquement | `services` | Installe Helm 3 sur le nœud master |
| `Deploy_Traefik` | Master uniquement | `services` | Déploie Traefik via Helm sur le nœud ingress |
| `Deploy_Monitoring` | Master uniquement | `services` | Déploie kube-prometheus-stack via Helm sur le nœud monitoring |
...
- name: Taint nodes
  ...
  environment:
    KUBECONFIG: /home/ec2-user/.kube/config
  loop: "{{ groups['all'] }}"
  when: hostvars[item]['server_role'] != 'master'
```
...
```bash
Déploiement complet (infra + services)
make setup

# Services uniquement (Traefik)
make deploy-Traefik
```
...
5. Deploy_Traefik     →  kube1 → pods sur nœud ingress
6. Deploy_Monitoring  →  kube1 → pods sur nœud monitoring
```
